...
#################### Model with minimum required controls, reported in the main text #################### 
...
param <- c('Alpha','BetaRich1','BetaRich2','BetaGood1','BetaGood2','BetaTrust1','BetaTrust2','BetaFriend','BetaBensBG','BetaBensWG','BetaBensC','BetaBensR','BetaCostBG','BetaCostWG','BetaCostC','BetaCostR','BetaCostD','BetaLive','BetaVis','BetaMed','BetaNony','BetaEthn','BetaPopI','BetaPopT','BetaVersA','BetaVersB','BetaVersC','BetaVersD','BetaVersE','sigmaZeta')
 ...
 write.csv(extr_sams,paste0("Minim_extracted samples_",gsub("^..","",dire),".csv"),na="",row.names=FALSE) #These will be necessary below to examine the posteriors.
 write.csv(summ_stat,paste0("Minim_summary statistics_",gsub("^..","",dire),".csv"),na="") #These are useful checks to have for looking at R hats, effective sample sizes, etc. Keep rownames here: they're useful to see which variables have low effective sample sizes or high R hats below.
 
}

######### CHECK R HATS AND EFFECTIVE SAMPLE SIZES ##########
fils_ss<-grep("./Minim_summary statistics_Iter",list.files(full.names = TRUE, recursive = TRUE)[-1],value=TRUE)

assig_leng<-names(read.csv("./Minim_summary statistics_Iter1.csv",header=T)) #Doesn't matter which one is picked, just need the column names.
...
library(xtable);library(forestplot);library(plotrix)

fils_es<-grep("./Minim_extracted samples_Iter",list.files(full.names = TRUE, recursive = TRUE)[-1],value=TRUE)
assig_leng<-names(read.csv("./Minim_extracted samples_Iter98.csv",header=T)) #Doesn't matter which one is picked, just need the column names.
...
# Reorder
stats2$Prediction<-c("","","P1.1",rep("",3),"P1.2",rep("",9),"P2.1",rep("",17),"P3.1",rep("",5),"Controls",rep("",7))
stats3<-stats2[,c(6,1,2,3,4,5)]

#################### FIGURE 3a
myColors<-c(rgb(223,194,125,1,maxColorValue=255),rgb(166,97,26,1,maxColorValue=255),rgb(128,205,193,1,maxColorValue=255),rgb(1,133,113,1,maxColorValue=255))
options(forestplot_new_page = FALSE)

pdf("Figure 3_31-Jul-17.pdf",width=6.30,height=6.30)
x <- unit(.05, 'npc'); y <- unit(.85, 'npc')
x1 <- unit(.05, 'npc'); y1 <- unit(.95, 'npc')

grid.newpage()
pushViewport(viewport(layout = grid.layout(2, 2)))
pushViewport(viewport(layout.pos.col = 1,layout.pos.row=1))

grid.text('A', x, y, gp = gpar(fontsize=20, font = 2))

forestplot(labeltext=c("A little\nwealthy","Wealthy"),
mean=cbind(stats2$Mean[stats2$Group=="IG" & stats2$Variable %in% c("A little wealthy","Wealthy")],stats2$Mean[stats2$Group=="OG" & stats2$Variable %in% c("A little wealthy","Wealthy")]),
lower=cbind(stats2$CI_5[stats2$Group=="IG" & stats2$Variable %in% c("A little wealthy","Wealthy")],stats2$CI_5[stats2$Group=="OG" & stats2$Variable %in% c("A little wealthy","Wealthy")]),upper=cbind(stats2$CI_95[stats2$Group=="IG" & stats2$Variable %in% c("A little wealthy","Wealthy")],stats2$CI_95[stats2$Group=="OG" & stats2$Variable %in% c("A little wealthy","Wealthy")]),
lty.zero=2,lwd.ci=3,lineheight=unit(3,"cm"),graphwidth=unit(4, "cm"),colgap=unit(6,"mm"),zero=1,boxsize=0.1,mar=unit(c(1,1,3,1),"lines"), xticks=seq(0.5,2,by=0.5),
col=fpColors(box=c(color.id(myColors[1]),color.id(myColors[3])),lines=c(color.id(myColors[1]),color.id(myColors[3]))),
txt_gp=fpTxtGp(label=gpar(fontsize=15),ticks=gpar(fontsize=20),xlab=gpar(fontsize=20),legend=gpar(fontsize=15)))#,legend=c("In-group","Out-group"),#legend_args=fpLegend(pos="top"))
#xlab="Relative Risk of Allocating\n1 More Boliviano",
popViewport()

#################### FIGURE 3b
pushViewport(viewport(layout.pos.col = 2,layout.pos.row=1))

grid.text('B', x, y, gp = gpar(fontsize=20, font = 2))

forestplot(labeltext=c("A little\ngood","Good"),
mean=cbind(stats2$Mean[stats2$Group=="IG" & stats2$Variable %in% c("A little good","Good")],stats2$Mean[stats2$Group=="OG" & stats2$Variable %in% c("A little good","Good")]),
lower=cbind(stats2$CI_5[stats2$Group=="IG" & stats2$Variable %in% c("A little good","Good")],stats2$CI_5[stats2$Group=="OG" & stats2$Variable %in% c("A little good","Good")]),upper=cbind(stats2$CI_95[stats2$Group=="IG" & stats2$Variable %in% c("A little good","Good")],stats2$CI_95[stats2$Group=="OG" & stats2$Variable %in% c("A little good","Good")]),
lty.zero=2,lwd.ci=3,lineheight=unit(3,"cm"),graphwidth=unit(4, "cm"),colgap=unit(6,"mm"),zero=1,boxsize=0.1,mar=unit(c(1,1.6,1,1),"lines"), xticks=seq(0.5,2,by=0.5),
col=fpColors(box=c(color.id(myColors[1]),color.id(myColors[3])),lines=c(color.id(myColors[1]),color.id(myColors[3]))),
txt_gp=fpTxtGp(label=gpar(fontsize=15),ticks=gpar(fontsize=20),xlab=gpar(fontsize=20),legend=gpar(fontsize=15)),legend=c("In-group","Out-group"),legend_args=fpLegend(pos="top"))
#xlab="Relative Risk of Allocating\n1 More Boliviano",
popViewport()
...
grid.text('C', x1, y1, gp = gpar(fontsize=20, font = 2))

forestplot(labeltext=c("A little\ntrustworthy","Trustworthy"),
mean=cbind(stats2$Mean[stats2$Group=="IG" & stats2$Variable %in% c("A little trustworthy","Trustworthy")],stats2$Mean[stats2$Group=="OG" & stats2$Variable %in% c("A little trustworthy","Trustworthy")]),
lower=cbind(stats2$CI_5[stats2$Group=="IG" & stats2$Variable %in% c("A little trustworthy","Trustworthy")],stats2$CI_5[stats2$Group=="OG" & stats2$Variable %in% c("A little trustworthy","Trustworthy")]),upper=cbind(stats2$CI_95[stats2$Group=="IG" & stats2$Variable %in% c("A little trustworthy","Trustworthy")],stats2$CI_95[stats2$Group=="OG" & stats2$Variable %in% c("A little trustworthy","Trustworthy")]),
lty.zero=2,lwd.ci=3,lineheight=unit(3,"cm"),graphwidth=unit(4, "cm"),colgap=unit(6,"mm"),zero=1,boxsize=0.1,mar=unit(c(2,0.5,1,1),"lines"), xticks=seq(0.5,2,by=0.5),
col=fpColors(box=c(color.id(myColors[1]),color.id(myColors[3])),lines=c(color.id(myColors[1]),color.id(myColors[3]))),
txt_gp=fpTxtGp(label=gpar(fontsize=12),ticks=gpar(fontsize=20),xlab=gpar(fontsize=20),legend=gpar(fontsize=15)),#legend=c("In-group","Out-group"),#legend_args=fpLegend(pos="top"))
xlab="Relative Risk of Allocating\n1 More Boliviano")
popViewport()
...
grid.text('D', x1, y1, gp = gpar(fontsize=20, font = 2))

forestplot(labeltext=c("Btwn-group\nbenefits","Btwn-group\ncosts"),
mean=cbind(stats2$Mean[stats2$Group=="IG" & stats2$Variable %in% c("Benefits: Btwn-group","Benefits: Within-group")],stats2$Mean[stats2$Group=="OG" & stats2$Variable %in% c("Benefits: Btwn-group","Benefits: Within-group")]),

lower=cbind(stats2$CI_5[stats2$Group=="IG" & stats2$Variable %in% c("Benefits: Btwn-group","Benefits: Within-group")],stats2$CI_5[stats2$Group=="OG" & stats2$Variable %in% c("Benefits: Btwn-group","Benefits: Within-group")]),

upper=cbind(stats2$CI_95[stats2$Group=="IG" & stats2$Variable %in% c("Benefits: Btwn-group","Benefits: Within-group")],stats2$CI_95[stats2$Group=="OG" & stats2$Variable %in% c("Benefits: Btwn-group","Benefits: Within-group")]),
...
pdf("Figure 4_31-Jul-17.pdf",width=3.54,height=4.72)

forestplot(labeltext=c("Places lived","Places visited","Hours of media"),
mean=cbind(stats2$Mean[stats2$Group=="IG" & stats2$Variable %in% c("Places lived","Places visited (z-score)","Hours of media")],stats2$Mean[stats2$Group=="OG" & stats2$Variable %in% c("Places lived","Places visited (z-score)","Hours of media")]),
lower=cbind(stats2$CI_5[stats2$Group=="IG" & stats2$Variable %in% c("Places lived","Places visited (z-score)","Hours of media")],stats2$CI_5[stats2$Group=="OG" & stats2$Variable %in% c("Places lived","Places visited (z-score)","Hours of media")]),upper=cbind(stats2$CI_95[stats2$Group=="IG" & stats2$Variable %in% c("Places lived","Places visited (z-score)","Hours of media")],stats2$CI_95[stats2$Group=="OG" & stats2$Variable %in% c("Places lived","Places visited (z-score)","Hours of media")]),
lty.zero=2,lwd.ci=3,lineheight=unit(3,"cm"),graphwidth=unit(4, "cm"),colgap=unit(6,"mm"),zero=1,boxsize=0.1,mar=unit(c(1,1,1,1),"lines"), xticks=seq(0.75,1.5,by=0.25),line.margin=unit(1.25,"cm"),
col=fpColors(box=c(color.id(myColors[1]),color.id(myColors[3])),lines=c(color.id(myColors[1]),color.id(myColors[3]))),
txt_gp=fpTxtGp(label=gpar(fontsize=10),ticks=gpar(fontsize=20),xlab=gpar(fontsize=17.5),legend=gpar(fontsize=15)),legend=c("In-group","Out-group"),legend_args=fpLegend(pos="top"),
xlab="Relative Risk of Allocating\n1 More Boliviano")
...
############### TABLE ###############
stats4<-stats3
names(stats4)[c(2,5,6)]<-c("Target group","Credible Int. 5%","Credible Int. 95%")
print(xtable(stats4),include.rownames=FALSE,type="html",file="Minimal table_27-Jul-17.html")