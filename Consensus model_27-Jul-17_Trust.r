...
#################### Model with CONSENSUS MEASURES, with trustworthiness included not goodness #################### 
...
//###################################################################### Model Full Data
 for(n in 1:N){
   for (k in 1:(K-1)){
	...
	BetaTrusto[Category[n,k]]*TrustoI[n,k] + BetaFriendo[Category[n,k]]*FriendoI[n,k] 
...
'BetaFriendo','BetaBensBG','BetaBensWG','BetaBensC','BetaBensR','BetaCostBG','BetaCostWG','BetaCostC','BetaCostR','BetaCostD','BetaLive','BetaVis','BetaMed','BetaNony','BetaEthn','BetaPopI','BetaPopT','BetaVersA','BetaVersB','BetaVersC','BetaVersD','BetaVersE','sigmaZeta')
 ...
 write.csv(extr_sams,paste0("Consensus_extracted samples_",gsub("^..","",dire),".csv"),na="",row.names=FALSE) #These will be necessary below to examine the posteriors.
 write.csv(summ_stat,paste0("Consensus_summary statistics_",gsub("^..","",dire),".csv"),na="") #These are useful checks to have for looking at R hats, effective sample sizes, etc. Keep rownames here: they're useful to see which variables have low effective sample sizes or high R hats below.
 
}

######### CHECK R HATS AND EFFECTIVE SAMPLE SIZES ##########
fils_ss<-grep("./Consensus_summary statistics_Iter",list.files(full.names = TRUE, recursive = TRUE)[-1],value=TRUE)

assig_leng<-names(read.csv("./Consensus_summary statistics_Iter1.csv",header=T)) #Doesn't matter which one is picked, just need the column names.
...
fils_es<-grep("./Consensus_extracted samples_Iter",list.files(full.names = TRUE, recursive = TRUE)[-1],value=TRUE)
assig_leng<-names(read.csv("./Consensus_extracted samples_Iter98.csv",header=T)) #Doesn't matter which one is picked, just need the column names.
...
print(xtable(stats3),include.rownames=FALSE,type="html",file="Consensus trust table_31-Jul-17.html")