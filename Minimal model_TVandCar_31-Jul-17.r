...
#################### Model with minimum required controls, including TV AND CAR OWNERSHIP #################### 
...
#TV and car
 TV<-as.matrix(read.csv(paste0(dire,"/TV.csv"),header=FALSE))
 Car<-as.matrix(read.csv(paste0(dire,"/Car.csv"),header=FALSE))
...
//###################################################################### Model Full Data
 for(n in 1:N){
   for (k in 1:(K-1)){
	...
	+ BetaTV[Category[n,k]]*TV[n,k] + BetaCar[Category[n,k]]*Car[n,k]
...
param <- c('Alpha','BetaRich1','BetaRich2','BetaGood1','BetaGood2','BetaTrust1','BetaTrust2','BetaFriend','BetaBensBG','BetaBensWG','BetaBensC','BetaBensR','BetaCostBG','BetaCostWG','BetaCostC','BetaCostR','BetaCostD','BetaLive','BetaVis','BetaMed','BetaTV','BetaCar','BetaNony','BetaEthn','BetaPopI','BetaPopT','BetaVersA','BetaVersB','BetaVersC','BetaVersD','BetaVersE','sigmaZeta')
 ...
 write.csv(extr_sams,paste0("TvCar_extracted samples_",gsub("^..","",dire),".csv"),na="",row.names=FALSE) #These will be necessary below to examine the posteriors.
 write.csv(summ_stat,paste0("TvCar_summary statistics_",gsub("^..","",dire),".csv"),na="") #These are useful checks to have for looking at R hats, effective sample sizes, etc. Keep rownames here: they're useful to see which variables have low effective sample sizes or high R hats below.
 
}

######### CHECK R HATS AND EFFECTIVE SAMPLE SIZES ##########
fils_ss<-grep("./TvCar_summary statistics_Iter",list.files(full.names = TRUE, recursive = TRUE)[-1],value=TRUE)

assig_leng<-names(read.csv("./TvCar_summary statistics_Iter1.csv",header=T)) #Doesn't matter which one is picked, just need the column names.
...
fils_es<-grep("./TvCar_extracted samples_Iter",list.files(full.names = TRUE, recursive = TRUE)[-1],value=TRUE)
assig_leng<-names(read.csv("./TvCar_extracted samples_Iter98.csv",header=T)) #Doesn't matter which one is picked, just need the column names.
...
stats2$Variable[grep("BetaTV",stats2$Variable)]<-"TV owned"
stats2$Variable[grep("BetaCar",stats2$Variable)]<-"Car owned"

# Reorder
stats2$Prediction<-c("","","P1.1",rep("",3),"P1.2",rep("",9),"P2.1",rep("",17),"P3.1",rep("",5),"Controls",rep("",11))
stats3<-stats2[,c(6,1,2,3,4,5)]

############### TABLE ###############
stats4<-stats3
names(stats4)[c(2,5,6)]<-c("Target group","Credible int. 5%","Credible int. 95%")
print(xtable(stats4),include.rownames=FALSE,type="html",file="TV Car table_31-Jul-17.html")