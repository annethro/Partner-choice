...
#################### Model with minimum required controls for MOSETEN #################### 
...
model_dat=list(
   ...
   Ethn=Eth,
   Vers=Ver
   ) #Pop has been deleted, since not relevant when just the Moseten.
...
// real<lower=0,upper=5> imp_liv; //# No imputation: missing data participant is Tsimane.
...
#,'sigmaZeta')
 ...
 write.csv(extr_sams,paste0("Moseten_extracted samples_",gsub("^..","",dire),".csv"),na="",row.names=FALSE) #These will be necessary below to examine the posteriors.
 write.csv(summ_stat,paste0("Moseten_summary statistics_",gsub("^..","",dire),".csv"),na="") #These are useful checks to have for looking at R hats, effective sample sizes, etc. Keep rownames here: they're useful to see which variables have low effective sample sizes or high R hats below.
 
}

######### CHECK R HATS AND EFFECTIVE SAMPLE SIZES ##########
fils_ss<-grep("./Moseten_summary statistics_Iter",list.files(full.names = TRUE, recursive = TRUE)[-1],value=TRUE)

assig_leng<-names(read.csv("./Moseten_summary statistics_Iter1.csv",header=T)) #Doesn't matter which one is picked, just need the column names.
...
fils_es<-grep("./Moseten_extracted samples_Iter",list.files(full.names = TRUE, recursive = TRUE)[-1],value=TRUE)
assig_leng<-names(read.csv("./Moseten_extracted samples_Iter98.csv",header=T)) #Doesn't matter which one is picked, just need the column names.
...
print(xtable(stats3),include.rownames=FALSE,type="html",file="Moseten table_31-Jul-17.html")