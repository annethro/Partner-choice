...
#################### Model with minimum required controls for INTERCULTURALES #################### 
...
model_dat=list(
   ...
   Ethn=Eth,
   Vers=Ver
   ) #Pop has been deleted, since not relevant when just the Interculturales.
...
// real<lower=0,upper=5> imp_liv; //# Imputation is for a Tsimane participant so not relevant here for Interculturales.
...
//###################################################################### Model Full Data
 for(n in 1:N){
   for (k in 1:(K-1)){
	...
	+ BetaLive[Category[n,k]]*NumLive[n,k] + BetaVis[Category[n,k]]*NumVis[n,k] + BetaMed[Category[n,k]]*Media[n,k]
...
param <- c('Alpha','BetaRich1','BetaRich2','BetaGood1','BetaGood2','BetaTrust1','BetaTrust2','BetaFriend','BetaBensBG','BetaBensWG','BetaBensC','BetaBensR','BetaCostBG','BetaCostWG','BetaCostC','BetaCostR','BetaCostD','BetaLive','BetaVis','BetaMed','BetaNony','BetaEthn',
##'BetaPopI','BetaPopT',
...
#,'sigmaZeta')
 ...
 write.csv(extr_sams,paste0("Intercultural_extracted samples_",gsub("^..","",dire),".csv"),na="",row.names=FALSE) #These will be necessary below to examine the posteriors.
 write.csv(summ_stat,paste0("Intercultural_summary statistics_",gsub("^..","",dire),".csv"),na="") #These are useful checks to have for looking at R hats, effective sample sizes, etc. Keep rownames here: they're useful to see which variables have low effective sample sizes or high R hats below.
 
}

######### CHECK R HATS AND EFFECTIVE SAMPLE SIZES ##########
fils_ss<-grep("./Intercultural_summary statistics_Iter",list.files(full.names = TRUE, recursive = TRUE)[-1],value=TRUE)

assig_leng<-names(read.csv("./Intercultural_summary statistics_Iter1.csv",header=T)) #Doesn't matter which one is picked, just need the column names.
...
fils_es<-grep("./Intercultural_extracted samples_Iter",list.files(full.names = TRUE, recursive = TRUE)[-1],value=TRUE)
assig_leng<-names(read.csv("./Intercultural_extracted samples_Iter98.csv",header=T)) #Doesn't matter which one is picked, just need the column names.
...
# Reorder
stats2$Prediction<-c("","","P1.1",rep("",3),"P1.2",rep("",9),"P2.1",rep("",17),"P3.1",rep("",5),"Controls",rep("",3))
stats3<-stats2[,c(6,1,2,3,4,5)]
names(stats3)[c(2,5,6)]<-c("Target group","Credible int. 5%","Credible int. 95%")

print(xtable(stats3),include.rownames=FALSE,type="html",file="Intercultural table_27-Jul-17.html")