...
#################### Model with controls that mattered for average out-group and in-group giving in Pisor & Gurven (2016) #################### 
...
param <- c('Alpha','BetaRich1','BetaRich2','BetaGood1','BetaGood2','BetaTrust1','BetaTrust2','BetaFriend','BetaBensBG','BetaBensWG','BetaBensC','BetaBensR','BetaCostBG','BetaCostWG','BetaCostC','BetaCostR','BetaCostD','BetaLive','BetaVis','BetaMed','BetaNony','BetaEthn','BetaPopI','BetaPopT','BetaVersA','BetaVersB','BetaVersC','BetaVersD','BetaVersE','sigmaZeta','BetaSES','BetaLabor','BetaIll')
 ...
 write.csv(extr_sams,paste0("Prev sig_extracted samples_",gsub("^..","",dire),".csv"),na="",row.names=FALSE) #These will be necessary below to examine the posteriors.
 write.csv(summ_stat,paste0("Prev sig_summary statistics_",gsub("^..","",dire),".csv"),na="") #These are useful checks to have for looking at R hats, effective sample sizes, etc. Keep rownames here: they're useful to see which variables have low effective sample sizes or high R hats below.
 
}

######### CHECK R HATS AND EFFECTIVE SAMPLE SIZES ##########
fils_ss<-grep("./Prev sig_summary statistics_Iter",list.files(full.names = TRUE, recursive = TRUE)[-1],value=TRUE)

assig_leng<-names(read.csv("./Prev sig_summary statistics_Iter1.csv",header=T)) #Doesn't matter which one is picked, just need the column names.
...
fils_es<-grep("./Prev sig_extracted samples_Iter",list.files(full.names = TRUE, recursive = TRUE)[-1],value=TRUE)
assig_leng<-names(read.csv("./Prev sig_extracted samples_Iter98.csv",header=T)) #Doesn't matter which one is picked, just need the column names.
...
# Reorder
stats2$Prediction<-c("","","P1.1",rep("",3),"P1.2",rep("",9),"P2.1",rep("",17),"P3.1",rep("",5),"Controls",rep("",7),"Additional variables",rep("",5))
stats3<-stats2[,c(6,1,2,3,4,5)]
names(stats3)[c(2,5,6)]<-c("Target group","Credible Int. 5%","Credible Int. 95%")

print(xtable(stats3),include.rownames=FALSE,type="html",file="Prev sig_30-Jul-17.html")