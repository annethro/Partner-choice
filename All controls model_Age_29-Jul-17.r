...
#################### Model with ALL candidate controls, with age not age difference included #################### 
...
#Full set of additional variables, except age difference
 ...
 Age <- as.matrix(read.csv(paste0(dire,"/Age.csv"),header=FALSE))
...
//###################################################################### Model Full Data
 for(n in 1:N){
   for (k in 1:(K-1)){
	...
	+ BetaAge[Category[n,k]]*Age[n,k] + BetaAgree[Category[n,k]]*Agree[n,k] + BetaChurX[Category[n,k]]*ChurX[n,k] + BetaEduc[Category[n,k]]*EducI[n,k] + BetaExtra[Category[n,k]]*Extra[n,k] + BetaInc[Category[n,k]]*Inc[n,k] + BetaMI[Category[n,k]]*MI[n,k] + BetaRisk[Category[n,k]]*Risk[n,k] + BetaSex[Category[n,k]]*Sex[n,k] + BetaShort[Category[n,k]]*Short[n,k] +  BetaStay[Category[n,k]]*Stay[n,k] 
...
param <- c('Alpha','BetaRich1','BetaRich2','BetaGood1','BetaGood2','BetaTrust1','BetaTrust2','BetaFriend','BetaBensBG','BetaBensWG','BetaBensC','BetaBensR','BetaCostBG','BetaCostWG','BetaCostC','BetaCostR','BetaCostD','BetaLive','BetaVis','BetaMed','BetaNony','BetaEthn','BetaPopI','BetaPopT','BetaVersA','BetaVersB','BetaVersC','BetaVersD','BetaVersE','sigmaZeta','BetaSES','BetaLabor','BetaIll','BetaAge','BetaAgree','BetaChurX','BetaEduc','BetaExtra','BetaInc','BetaMI','BetaRisk','BetaSex','BetaShort','BetaStay')
 ...
 write.csv(extr_sams,paste0("All controls_extracted samples_",gsub("^..","",dire),".csv"),na="",row.names=FALSE) #These will be necessary below to examine the posteriors.
 write.csv(summ_stat,paste0("All controls_summary statistics_",gsub("^..","",dire),".csv"),na="") #These are useful checks to have for looking at R hats, effective sample sizes, etc. Keep rownames here: they're useful to see which variables have low effective sample sizes or high R hats below.
 
}

######### CHECK R HATS AND EFFECTIVE SAMPLE SIZES ##########
fils_ss<-grep("./All controls_summary statistics_Iter",list.files(full.names = TRUE, recursive = TRUE)[-1],value=TRUE)

assig_leng<-names(read.csv("./All controls_summary statistics_Iter1.csv",header=T)) #Doesn't matter which one is picked, just need the column names.
...
fils_es<-grep("./All controls_extracted samples_Iter",list.files(full.names = TRUE, recursive = TRUE)[-1],value=TRUE)
assig_leng<-names(read.csv("./All controls_extracted samples_Iter98.csv",header=T)) #Doesn't matter which one is picked, just need the column names.
...
stats2$Variable[grep("BetaAge",stats2$Variable)]<-"Donor age"
...
print(xtable(stats3),include.rownames=FALSE,type="html",file="All controls_Age_30-Jul-17.html")